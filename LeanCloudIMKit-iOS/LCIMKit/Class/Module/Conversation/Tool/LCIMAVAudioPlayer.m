...
@interface LCIMAVAudioPlayer () <AVAudioPlayerDelegate,AVAudioSessionDelegate>
{
...
@implementation LCIMAVAudioPlayer
...
- (instancetype)init {
    if ([super init]) {
        ...
        _audioDataOperationQueue.name = @"com.LeanCloud.LCIMAVAudipPlayer.loadAudioDataQueue";
...
- (void)playAudioWithURLString:(NSString *)URLString atIndex:(NSUInteger)index{
    ...
    NSBlockOperation *blockOperation = [NSBlockOperation blockOperationWithBlock:^{
        [self setAudioPlayerState:LCIMVoiceMessageStateDownloading];
...
- (void)playAudioWithData:(NSData *)audioData {
    ...
    [self setAudioPlayerState:LCIMVoiceMessageStatePlaying];
    [_audioPlayer play];
...
- (NSString *)cachePath {
    ...
    if (!_cachePath) {
        _cachePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"com.LeanCloud.LCIMChat.audioCache"];
...
- (void)setAudioPlayerState:(LCIMVoiceMessageState)audioPlayerState {
    ...
    if (_audioPlayerState == LCIMVoiceMessageStateCancel || _audioPlayerState == LCIMVoiceMessageStateNormal) {
        _URLString = nil;
        _index = NSUIntegerMax;
    }
}
