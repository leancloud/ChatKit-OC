...
- (instancetype)init{
    if (self = [super init]) {
        ...
        NSArray *faceArray = [NSArray arrayWithContentsOfFile:[LCIMFaceManager defaultEmojiFacePath]];
...
+ (NSArray *)emojiFaces {
    return [[LCIMFaceManager shareInstance] emojiFaceArrays];
}
...
+ (NSString *)faceImageNameWithFaceID:(NSUInteger)faceID {
    ...
    for (NSDictionary *faceDict in [[LCIMFaceManager shareInstance] emojiFaceArrays]) {
...
+ (NSMutableAttributedString *)emotionStrWithString:(NSString *)text {
    ...
    for(NSTextCheckingResult *match in resultArray) {
        ...
        for (NSDictionary *dict in [[LCIMFaceManager shareInstance] emojiFaceArrays]) {
            if ([dict[kFaceNameKey]  isEqualToString:subStr]) {
                //face[i][@"png"]就是我们要加载的图片
                //新建文字附件来存放我们的图片,iOS7才新加的对象
                NSTextAttachment *textAttachment = [[NSTextAttachment alloc] init];
                //给附件添加图片
                textAttachment.image = [UIImage imageNamed:[self imageBundleNameForImage:dict[kFaceImageNameKey]]];
 ...
 */
+ (NSArray *)recentFaces{
    return [[LCIMFaceManager shareInstance] recentFaceArrays];
}


+ (BOOL)saveRecentFace:(NSDictionary *)recentDict{
    for (NSDictionary *dict in [[LCIMFaceManager shareInstance] recentFaceArrays]) {
        if ([dict[@"face_id"] integerValue] == [recentDict[@"face_id"] integerValue]) {
            NSLog(@"已经存在");
            return NO;
        }
    }
    [[[LCIMFaceManager shareInstance] recentFaceArrays] insertObject:recentDict atIndex:0];
    if ([[LCIMFaceManager shareInstance] recentFaceArrays].count > 8) {
        [[[LCIMFaceManager shareInstance] recentFaceArrays] removeLastObject];
    }
    [[NSUserDefaults standardUserDefaults] setObject:[[LCIMFaceManager shareInstance] recentFaceArrays] forKey:@"recentFaceArrays"];
    return YES;
}
