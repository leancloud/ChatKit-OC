...
@interface LCIMConversationViewModel ()

@property (nonatomic, weak) LCIMConversationViewController *parentViewController;
...
@implementation LCIMConversationViewModel
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LCIMChatMessageCell *messageCell = [tableView dequeueReusableCellWithIdentifier:identifier forIndexPath:indexPath];
...
- (void)sendMessage:(LCIMMessage *)message {
    ...
    [self sendMessage:message
        progressBlock:^(NSInteger percentDone) {
            [self.delegate messageSendStateChanged:LCIMMessageSendStateSending withProgress:percentDone/100.f forIndex:[self.dataArray indexOfObject:message]];
        }
...
- (void)sendMessage:(LCIMMessage *)message
      progressBlock:(AVProgressBlock)progressBlock
            success:(LCIMSendMessageSuccessBlock)success
             failed:(LCIMSendMessageSuccessFailedBlock)failed {
    ...
    id<LCIMUserModelDelegate> sender = [[LCIMUserSystemService sharedInstance] fetchCurrentUser];
    message.avatorURL = sender.avatorURL;
    message.bubbleMessageType = LCIMMessageOwnerSelf;
    AVIMTypedMessage *avimTypedMessage = [LCIMConversationViewModel getAVIMTypedMessageWithMessage:message];
    ...
    [[LCIMConversationService sharedInstance] sendMessage:avimTypedMessage
                                             conversation:[LCIMConversationService sharedInstance].currentConversation
...
+ (LCIMMessage *)getLCIMMessageByMessage:(AVIMTypedMessage *)message {
    ...
    switch (mediaType) {
        case kAVIMMessageMediaTypeText: {
            ...
            lcimMessage = [[LCIMMessage alloc] initWithText:textMsg.text sender:fromUser.name timestamp:time];
            break;
        }
        case kAVIMMessageMediaTypeAudio: {
            ...
            lcimMessage = [[LCIMMessage alloc] initWithVoicePath:voicePath voiceURL:nil voiceDuration:duration sender:fromUser.name timestamp:time];
            break;
        }
        ...
        case kAVIMMessageMediaTypeImage: {
            ...
            lcimMessage = [[LCIMMessage alloc] initWithPhoto:nil thumbnailPhoto:nil photoPath:imagePath thumbnailURL:nil originPhotoURL:[NSURL URLWithString:imageMsg.file.url] sender:fromUser.name timestamp:time];
            break;
        }
...
- (void)loadMessagesWhenInitHandler:(LCIMBooleanResultBlock)handler {
    if (self.parentViewController.loadingMoreMessage) {
        ...
        [[self class] queryAndCacheMessagesWithTimestamp:([[NSDate distantFuture] timeIntervalSince1970] * 1000) block:^(NSArray *avimTypedMessage, NSError *error) {
            ...
            if (succeed) {
                if (avimTypedMessage.count < kLCIMOnePageSize) {
                ...
                NSMutableArray *lcimSucceedMessags = [LCIMConversationViewModel getLCIMMessages:avimTypedMessage];
