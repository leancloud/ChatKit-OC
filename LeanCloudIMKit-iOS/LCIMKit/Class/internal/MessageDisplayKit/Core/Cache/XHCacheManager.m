...
- (NSString *)_cacheDirectory {
    if(_cacheDirectoryPath == nil) {
        ...
        _cacheDirectoryPath = [paths.lastObject stringByAppendingPathComponent:self.identifier.lcim_MD5Hash];
...
- (void)removeCacheForURL:(NSURL *)url {
    if(url.absoluteString.length > 0){
        [self _removeCacheForHash:url.absoluteString.lcim_MD5Hash];
    }
}
...
- (void)storeData:(NSData *)data forURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    if(data && url.absoluteString.length > 0) {
        [self _storeData:data forHash:url.absoluteString.lcim_MD5Hash storeMemoryCache:storeMemoryCache];
    }
}
...
- (NSData *)localCachedDataWithURL:(NSURL *)url {
    if(url.absoluteString.length > 0){
        return [self _localCachedDataWithHash:url.absoluteString.lcim_MD5Hash];
    }
    return nil;
}
...
- (NSData *)dataWithURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    ...
    return [self _cachedDataWithHash:url.absoluteString.lcim_MD5Hash storeMemoryCache:storeMemoryCache];
}

- (BOOL)existsDataForURL:(NSURL *)url {
    if(url.absoluteString.length > 0) {
        NSString *path = [self _pathForHash:url.absoluteString.lcim_MD5Hash];
...
- (void)storeMemoryCacheWithImage:(UIImage *)image forURL:(NSURL *)url {
    if(image && url.absoluteString.length > 0) {
        [self storeMemoryCacheWithImage:image forHash:url.absoluteString.lcim_MD5Hash];
    }
}
...
- (UIImage *)imageWithURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    ...
    if(data) {
        ...
        if([data isKindOfClass:[NSData class]]) {
            image = [UIImage lcim_fastImageWithData:data];
