...
@property (strong, nonatomic) LCIMChatFaceView *faceView; /**< 当前活跃的底部view,用来指向faceView */
@property (strong, nonatomic) LCIMChatMoreView *moreView; /**< 当前活跃的底部view,用来指向moreView */
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    CGFloat offset = 10;
    textView.scrollEnabled = (textSize.height + 0.1 > kLCIMChatBarMaxHeight-offset);
    textViewFrame.size.height = MAX(34, MIN(kLCIMChatBarMaxHeight, textSize.height));
...
#pragma mark - LCIMLocationControllerDelegate
...
- (void)endConvertWithMP3FileName:(NSString *)fileName {
    if (fileName) {
        [LCIMProgressHUD dismissWithProgressState:LCIMProgressSuccess];
        [self sendVoiceMessage:fileName seconds:[LCIMProgressHUD seconds]];
    } else{
        [LCIMProgressHUD dismissWithProgressState:LCIMProgressError];
    }
}
...
- (void)beginConvert {
    NSLog(@"开始转换");
    [LCIMProgressHUD changeSubTitle:@"正在转换..."];
}

#pragma mark - LCIMChatMoreViewDelegate & LCIMChatMoreViewDataSource

- (void)moreView:(LCIMChatMoreView *)moreView selectIndex:(LCIMChatMoreItemType)itemType{
    switch (itemType) {
        case LCIMChatMoreItemAlbum:
        {
            //显示相册
        ...
        case LCIMChatMoreItemCamera:
        {
            ...
            break;
        case LCIMChatMoreItemLocation:
        {
            //显示地理位置
            LCIMLocationController *locationC = [[LCIMLocationController alloc] init];
...
- (NSArray<NSString *> *)imageNamesOfMoreView:(LCIMChatMoreView *)moreView{
    return @[@"chat_bar_icons_camera", @"chat_bar_icons_pic", @"chat_bar_icons_location"];
}

#pragma mark - LCIMChatFaceViewDelegate
...
- (void)faceViewSendFace:(NSString *)faceName{
    if ([faceName isEqualToString:@"[删除]"]) {
        ...
        self.inputText = @"";
        self.textView.text = @"";
        [self setFrame:CGRectMake(0, self.superViewHeight - self.bottomHeight - kLCIMChatBarMinHeight, self.frame.size.width, kLCIMChatBarMinHeight) animated:NO];
        [self showViewWithType:LCIMFunctionViewShowFace];
    } else{
...
- (void)endInputing{
    [self showViewWithType:LCIMFunctionViewShowNothing];
}
...
- (void)cancelRecordVoice{
    [LCIMProgressHUD dismissWithMessage:@"取消录音"];
...
- (void)updateCancelRecordVoice{
    [LCIMProgressHUD changeSubTitle:@"松开取消录音"];
}
...
- (void)updateContinueRecordVoice{
    [LCIMProgressHUD changeSubTitle:@"向上滑动取消录音"];
}


- (void)showViewWithType:(LCIMFunctionViewShowType)showType{
    
    //显示对应的View
    [self showMoreView:showType == LCIMFunctionViewShowMore && self.moreButton.selected];
    [self showVoiceView:showType == LCIMFunctionViewShowVoice && self.voiceButton.selected];
    [self showFaceView:showType == LCIMFunctionViewShowFace && self.faceButton.selected];
    
    switch (showType) {
        case LCIMFunctionViewShowNothing:
        case LCIMFunctionViewShowVoice:
        {
            self.inputText = self.textView.text;
            self.textView.text = nil;
            [self setFrame:CGRectMake(0, self.superViewHeight - kLCIMChatBarMinHeight, self.frame.size.width, kLCIMChatBarMinHeight) animated:NO];
            [self.textView resignFirstResponder];
        }
            break;
        case LCIMFunctionViewShowMore:
        case LCIMFunctionViewShowFace:
            self.inputText = self.textView.text;
            ...
            [self textViewDidChange:self.textView];
            break;
        case LCIMFunctionViewShowKeyboard:
...
- (void)buttonAction:(UIButton *)button{
    self.inputText = self.textView.text;
    LCIMFunctionViewShowType showType = button.tag;
    ...
    if (!button.selected) {
        showType = LCIMFunctionViewShowKeyboard;
...
- (void)sendTextMessage:(NSString *)text{
    ...
    self.textView.text = @"";
    [self setFrame:CGRectMake(0, self.superViewHeight - self.bottomHeight - kLCIMChatBarMinHeight, self.frame.size.width, kLCIMChatBarMinHeight) animated:NO];
    [self showViewWithType:LCIMFunctionViewShowKeyboard];
}
...
- (LCIMChatFaceView *)faceView{
    if (!_faceView) {
        _faceView = [[LCIMChatFaceView alloc] initWithFrame:CGRectMake(0, self.superViewHeight , self.frame.size.width, kFunctionViewHeight)];
...
- (LCIMChatMoreView *)moreView{
    if (!_moreView) {
        _moreView = [[LCIMChatMoreView alloc] initWithFrame:CGRectMake(0, self.superViewHeight, self.frame.size.width, kFunctionViewHeight)];
...
- (UIButton *)voiceButton{
    if (!_voiceButton) {
        ...
        _voiceButton.tag = LCIMFunctionViewShowVoice;
...
- (UIButton *)moreButton{
    if (!_moreButton) {
        ...
        _moreButton.tag = LCIMFunctionViewShowMore;
...
- (UIButton *)faceButton{
    if (!_faceButton) {
        ...
        _faceButton.tag = LCIMFunctionViewShowFace;
