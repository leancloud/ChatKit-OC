...
- (void)setupMessageInputViewBarWithStyle:(XHMessageInputViewStyle)style {
    ...
    if (self.allowsSendVoice) {
        //TODO: add method to create image name with bundle path.
        button = [self createButtonWithImage:({
            NSString *imageName = @"ToolViewInputVoice";
            ...
            image;})
                                     HLImage:({
            NSString *imageName = @"ToolViewInputVoiceHL";
            NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", imageName];
            UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
            image;})
                  ];
        ...
        button.tag = 0;
        [button setImage:({
            NSString *imageName = @"ToolViewKeyboard";
            NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", imageName];
            UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
            image;})
    ...
    if (self.allowsSendMultiMedia) {
        button = [self createButtonWithImage:({
            NSString *iconName = @"TypeSelectorBtn_Black";
            NSString *imageString = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", iconName];
            UIImage *image = [UIImage imageNamed:imageString];
            image;})
                                     HLImage:({
            NSString *iconNameHighLighted = @"TypeSelectorBtnHL_Black";
            NSString *imageStringHighLighted = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", iconNameHighLighted];
            UIImage *imageHighLighted = [UIImage imageNamed:imageStringHighLighted];
            imageHighLighted;
        })];
        [button addTarget:self action:@selector(messageStyleButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        button.tag = 2;
    ...
    if (self.allowsSendFace) {
        button = [self createButtonWithImage:({
            NSString *imageName = @"ToolViewEmotion";
            NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", imageName];
            UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
            image;})
                                     HLImage:({
            NSString *imageName = @"ToolViewEmotionHL";
            NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", imageName];
            UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
            image;})
                  ];
    ...
    switch (style) {
        case XHMessageInputViewStyleQuasiphysical: {
            ...
            NSString *imageName = @"input_bar_background";
            NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", imageName];
            UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
            self.image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(19.0f, 3.0f, 19.0f, 3.0f)
            ...
            inputFieldBack.image = [({
                NSString *imageName = @"input_field_cover";
                NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", imageName];
                UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
                image;
            }) resizableImageWithCapInsets:UIEdgeInsetsMake(20.0f, 12.0f, 18.0f, 18.0f)];
        ...
        case XHMessageInputViewStyleFlat: {
            ...
            self.image = [({
                NSString *imageName = @"input_bar_flat";
                NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"ChatKeyboard.bundle/%@", imageName];
                UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
                image;})
    ...
    if (self.allowsSendVoice) {
        ...
        button = [self createButtonWithImage:XH_STRETCH_IMAGE(({
            NSString *imageName = @"VoiceBtn_Black";
            NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"VoiceMessageSource.bundle/%@", imageName];
            UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
            image;}), edgeInsets)
                                     HLImage:XH_STRETCH_IMAGE(({
            NSString *imageName = @"VoiceBtn_BlackHL";
            NSString *imageNameWithBundlePath = [NSString stringWithFormat:@"VoiceMessageSource.bundle/%@", imageName];
            UIImage *image = [UIImage imageNamed:imageNameWithBundlePath];
            image;}), edgeInsets)];
...
- (void)adjustTextViewHeightBy:(CGFloat)changeInHeight {
    ...
    NSUInteger numLines = MAX([self.inputTextView numberOfLinesOfText],
                              [self.inputTextView.text lcim_numberOfLines]);
