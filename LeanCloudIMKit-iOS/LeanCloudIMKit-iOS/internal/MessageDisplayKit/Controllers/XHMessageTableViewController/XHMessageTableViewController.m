...
static CGFloat const LCIMScrollViewInsetTop = 0.f;
...
- (void)initilzer {
    ...
    if (self.allowsPanToDismissKeyboard) {
        ...
        self.messageTableView.lcim_keyboardDidScrollToPoint = ^(CGPoint point) {
            if (weakSelf.textViewInputViewType == XHInputViewTypeText)
                AnimationForMessageInputViewAtPoint(point);
        };
        
        self.messageTableView.lcim_keyboardWillSnapBackToPoint = ^(CGPoint point) {
            if (weakSelf.textViewInputViewType == XHInputViewTypeText)
                AnimationForMessageInputViewAtPoint(point);
        };
        
        self.messageTableView.lcim_keyboardWillBeDismissed = ^() {
            CGRect inputViewFrame = weakSelf.messageInputView.frame;
    ...
    // block回调键盘通知
    self.messageTableView.lcim_keyboardWillChange = ^(CGRect keyboardRect, UIViewAnimationOptions options, double duration, BOOL showKeyborad) {
        if (weakSelf.textViewInputViewType == XHInputViewTypeText) {
    ...
    self.messageTableView.lcim_keyboardDidChange = ^(BOOL didShowed) {
    ...
    self.messageTableView.lcim_keyboardDidHide = ^() {
    ...
    self.messageTableView.lcim_messageInputBarHeight = CGRectGetHeight(_messageInputView.bounds);
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!self.observingContentOffsetAndPanGestureControl) {
        ...
        // 设置键盘通知或者手势控制键盘消失
        [self.messageTableView lcim_setupPanGestureControlKeyboardHide:self.allowsPanToDismissKeyboard];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (self.observingContentOffsetAndPanGestureControl) {
        // remove KVO
        // remove键盘通知或者手势
        [self.messageTableView lcim_disSetupPanGestureControlKeyboardHide:self.allowsPanToDismissKeyboard];
