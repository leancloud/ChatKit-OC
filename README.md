...
### CocoaPods 导入
 ...
 ```
  ...
  打开 Podfile，在您项目的 target 下加入以下内容。（在此以 v0.8.1 版本为例）
...
`>= 0.8.1` 会根据您本地的 CocoaPods 源列表，导入不低于 0.8.1 版本的 SDK。
`~> 0.8.1` 会根据您本地的 CocoaPods 源列表，介于 0.7.X~0.8.1 之前版本的 SDK。
我们建议您锁定版本，便于团队开发。如，指定 0.8.1 版本。
