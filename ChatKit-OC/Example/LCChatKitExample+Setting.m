...
#endif
    /**
     * @attention 请区别 `[AVOSCloud setApplicationId:appId clientKey:appKey];` 与 `[LCChatKit
     setAppId:appId appKey:appKey];`。
    ...
    [[LCChatKit sharedInstance]
        setFetchProfilesBlock:^(NSArray<NSString *> *userIds,
          ...
          if (userIds.count == 0) {
              ...
              NSError *error =
                  [NSError errorWithDomain:@"LCChatKitExample" code:code userInfo:errorInfo];
...
- (void)lcck_setupCustomCellForConversationList
{
    // 自定义Cell菜单
    [[LCChatKit sharedInstance]
        setConversationEditActionBlock:^NSArray *(
          ...
          return [self exampleConversationEditActionAtIndexPath:indexPath
...
- (void)lcck_setupConversationsCellOperation
{
    ...
    [[LCChatKit sharedInstance] setDidSelectConversationsListCellBlock:^(
      ...
      [[self class]
          exampleOpenConversationViewControllerWithConversaionId:conversation.conversationId
...
{
    [[LCChatKit sharedInstance]
        setConversationInvalidedHandler:^(NSString *conversationId,
...
{
    [[LCChatKit sharedInstance]
        setPreviewImageMessageBlock:^(NSUInteger index, NSArray *allVisibleImages,
...
- (void)lcck_setupForceReconect
{
    ...
    [[LCChatKit sharedInstance] setForceReconnectSessionBlock:^(
      ...
      if (granted == YES) {
          ...
          [[LCChatKit sharedInstance]
              openWithClientId:[LCChatKit sharedInstance].clientId
                         force:force
...
{
    [[LCChatKit sharedInstance]
        setHUDActionBlock:^(UIViewController *viewController, UIView *view, NSString *title,
...
{
    [[LCChatKit sharedInstance]
        setAvatarImageViewCornerRadiusBlock:^CGFloat(CGSize avatarImageViewSize) {
...
- (void)lcck_setupFilterMessage
{
    ...
    [[LCChatKit sharedInstance]
        setFilterMessagesBlock:^(AVIMConversation *conversation,
...
{
    [[LCChatKit sharedInstance]
        setShowNotificationBlock:^(UIViewController *viewController, NSString *title,
...
{
    [[LCChatKit sharedInstance]
        setPreviewLocationMessageBlock:^(CLLocation *location, NSString *geolocations,
                                         NSDictionary *userInfo) {
          [self examplePreViewLocationMessageWithLocation:location geolocations:geolocations];
        }];
}
#pragma mark - private
- (void)exampleShowNotificationWithTitle:(NSString *)title
...
- (void)examplePreViewLocationMessageWithLocation:(CLLocation *)location
 ...
 */
- (void)exampleMarkBadgeWithTotalUnreadCount:(NSInteger)totalUnreadCount
                                         ...
                                         lcck_setObject:
                                             LCCKTestConversationGroupAvatarURLs[arc4random_uniform(
                                               ...
                                               callback:^(BOOL succeeded, NSError *error) {
                                                 [self lcck_hideHUD];
                                                 if (succeeded) {
                                                     [self lcck_showSuccess:@"设"
                                                                            @"置群头像成功"];
                                                     if (shouldInsert) {
                                                         [[LCChatKit sharedInstance]
                                                             insertRecentConversation:conversation];
                                                     }
                          ...
                          controller:(LCCKConversationListViewController *)controller
{
    ...
    UITableViewRowAction *actionItemDelete = [UITableViewRowAction
        rowActionWithStyle:UITableViewRowActionStyleDefault
                   ...
                   handler:^(UITableViewRowAction *action, NSIndexPath *indexPath) {
                     [[LCChatKit sharedInstance]
                         deleteRecentConversationWithConversationId:conversation.conversationId];
                   }];
    ...
    UITableViewRowAction *actionItemChangeGroupAvatar = [UITableViewRowAction
        rowActionWithStyle:UITableViewRowActionStyleDefault
                   ...
                   handler:^(UITableViewRowAction *action, NSIndexPath *indexPath) {
                     [[self class]
                         exampleChangeGroupAvatarURLsForConversationId:conversation.conversationId
                                                          shouldInsert:NO];
                   }];
 ...
 *
 /
 - (void)examplePreviewImageMessageWithInitialIndex:(NSUInteger)index allVisibleImages:(NSArray
 *)allVisibleImages allVisibleThumbs:(NSArray *)allVisibleThumbs {
 // Browser
